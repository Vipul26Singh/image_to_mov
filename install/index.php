...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <?php if(is_writable($db_config_path)){?>
		...
		<?php
		$ffmpeg_exists = null;
                exec("which ffmpeg 2>&1", $ffmpeg_exists);

                if(!empty($ffmpeg_exists)){ ?>
		...
		<?php }else{ ?>
                <p class="error">ffmpeg is missing</p>
        <?php } ?>
		<?php }else{ ?>
                <p class="error">Please make the uploads/ writable. </p>
        <?php } ?>
	  ...
	  <?php } else { ?>
		...
		<?php $ffmpeg_exists = exec("which ffmpeg");
      ...
      <p class="error">Please make the application/config/database.php file writable. </p>
	  <?php } ?>
